...
import { type Pattern, any, number, text, type Path } from "../src";
import { formatPaths } from "../src/format";
import type { Program, Instr, Axis } from "../src/pattern/vm";
import { runProgram as run } from "../src/pattern";
import { diagnostic } from "@blockchaincommons/dcbor/diagnostic";

/**
 * Runs a program and returns its paths and captures as a pair.
 */
const runVm = (
  program: Program,
...
describe("vm navigation tests", () => {
  ...
  it("test_vm_multiple_captures", () => {
    ...
    // children are pushed in source order, so the stack pops them in reverse
