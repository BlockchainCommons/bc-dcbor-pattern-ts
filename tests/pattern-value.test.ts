...
describe("value pattern tests", () => {
  describe("bool patterns", () => {
    it("test_bool_pattern_any", () => {
      const pattern = parsePattern("bool");
    ...
    it("test_bool_pattern_display", () => {
      expect(display(parsePattern("bool"))).toBe("bool");
      expect(display(parsePattern("true"))).toBe("true");
      expect(display(parsePattern("false"))).toBe("false");
    });
  });
  ...
  describe("text patterns", () => {
    ...
    it("test_text_pattern_specific", () => {
      const helloPattern = parsePattern('"Hello"');
      const worldPattern = parsePattern('"World"');
    ...
    it("test_text_pattern_display", () => {
      expect(display(parsePattern("text"))).toBe("text");
      expect(display(parsePattern('"Hello"'))).toBe('"Hello"');

      const regexPattern = textRegex(/^\d+$/);
      expect(display(regexPattern)).toBe("/^\\d+$/");
    });
  });

  describe("number patterns", () => {
    it("test_number_pattern_any", () => {
      const pattern = parsePattern("number");
    ...
    it("test_number_pattern_specific", () => {
      const intPattern = parsePattern("42");
      const floatPattern = parsePattern("3.2222");
    ...
    it("test_number_pattern_range", () => {
      const rangePattern = parsePattern("10...20");
    ...
    it("test_number_pattern_comparisons", () => {
      const gtPattern = parsePattern(">10");
      const gtePattern = parsePattern(">=10");
      const ltPattern = parsePattern("<10");
      const ltePattern = parsePattern("<=10");
    ...
    it("test_number_pattern_nan", () => {
      const nanPattern = parsePattern("NaN");
    ...
    it("test_number_pattern_display", () => {
      expect(display(parsePattern("number"))).toBe("number");
      expect(display(parsePattern("42"))).toBe("42");
      expect(display(parsePattern("3.2222"))).toBe("3.2222");
      expect(display(parsePattern("10...20"))).toBe("10...20");
      expect(display(parsePattern(">10"))).toBe(">10");
      expect(display(parsePattern(">=10"))).toBe(">=10");
      expect(display(parsePattern("<10"))).toBe("<10");
      expect(display(parsePattern("<=10"))).toBe("<=10");
      expect(display(parsePattern("NaN"))).toBe("NaN");
    });
  });

  describe("bytestring patterns", () => {
    it("test_byte_string_pattern_any", () => {
      const pattern = parsePattern("bstr");
    ...
    it("test_byte_string_pattern_specific", () => {
      const exactPattern = parsePattern("h'01020304'");
      const differentPattern = parsePattern("h'0506'");
    ...
    it("test_byte_string_pattern_display", () => {
      expect(display(parsePattern("bstr"))).toBe("bstr");
      expect(display(parsePattern("h'deadbeef'"))).toBe("h'deadbeef'");

      const regexPattern = byteStringRegex(/^test.*/);
      expect(display(regexPattern)).toBe("h'/^test.*/'");
    });
  });
  ...
  describe("date patterns", () => {
    it("test_date_pattern_any", () => {
      ...
      const pattern = parsePattern("date");
    ...
    it("test_date_pattern_display", () => {
      registerTags(getGlobalTagsStore());
      expect(display(parsePattern("date"))).toBe("date");

      const date = CborDate.fromYmd(2023, 12, 25);
      expect(display(datePattern(date))).toBe(`date'${date.toString()}'`);

      const startDate = CborDate.fromYmd(2023, 12, 20);
      const endDate = CborDate.fromYmd(2023, 12, 30);
      expect(display(dateRange(startDate, endDate))).toBe(
        `date'${startDate.toString()}...${endDate.toString()}'`,
      );

      expect(display(dateEarliest(date))).toBe(`date'${date.toString()}...'`);

      expect(display(dateLatest(date))).toBe(`date'...${date.toString()}'`);

      expect(display(dateIso8601("2023-12-25T00:00:00Z"))).toBe("date'2023-12-25T00:00:00Z'");

      const regex = /^2023-/;
      expect(display(dateRegex(regex))).toBe("date'/^2023-/'");
    });
  });

  describe("known value patterns", () => {
    it("test_known_value_pattern_any", () => {
      const pattern = parsePattern("known");
    ...
    it("test_known_value_pattern_specific", () => {
      ...
      const isAPattern = parsePattern("'isA'");
      const dateValPattern = parsePattern("'date'");
    ...
    it("test_known_value_pattern_named", () => {
      ...
      const isAPattern = parsePattern("'isA'");
      const dateValPattern = parsePattern("'date'");
      const unknownPattern = parsePattern("'unknownValue'");
  ...
  describe("null patterns", () => {
    ...
    it("test_null_pattern_display", () => {
      expect(display(parsePattern("null"))).toBe("null");
    });
  });

  describe("map patterns with various key types", () => {
    it("test_map_int_keys", () => {
      const mapPattern = parsePattern("{1: text}");
    ...
    it("test_map_bool_keys", () => {
      const mapPattern = parsePattern("{true: text}");
    ...
    it("test_map_complex_keys", () => {
      const mapPattern = parsePattern('{"a"|"b": text}');
    ...
    it("test_map_float_keys", () => {
      const mapPattern = parsePattern("{3.2222: text}");
    ...
    it("test_map_bstr_keys", () => {
      const mapPattern = parsePattern("{h'68656c6c6f': text}");
    ...
    it("test_map_known_value_keys", () => {
      registerTags(getGlobalTagsStore());
      const mapPattern = parsePattern("{'100': text}");
  ...
  describe("array patterns", () => {
    it("test_array_complex_elements", () => {
      ...
      const sequencePattern = parsePattern('[(1|2), "hello"]');
      ...
      const complexPattern = parsePattern('["hello", "a"|"b"]');
    ...
    it("test_array_or_precedence_issue", () => {
      ...
      const pattern = parsePattern('[1|2, "hello"]');
