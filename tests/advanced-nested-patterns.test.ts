/**
 * Advanced nested pattern tests.
 */

import { describe, it, expect } from "vitest";
import { cbor, taggedValue } from "@blockchaincommons/dcbor";
import { parsePattern, matches, getPaths, formatPathsStr, assertActualExpected } from "./common";

describe("advanced nested patterns", () => {
  it("test_simple_nested_tagged_array", () => {
    const pattern = parsePattern(`tagged(100, ["target"])`);
  ...
  it("test_map_with_array_constraints", () => {
    const pattern = parsePattern(`{"users": [{3,}]}`);
  ...
  it("test_array_starting_with_maps", () => {
    const pattern = parsePattern(`[{"id": number}, (*)*]`);
  ...
  it("test_deeply_nested_structures", () => {
    const pattern = parsePattern(`tagged(200, {"data": [{"value": number}]})`);
  ...
  it("test_multiple_levels_of_nesting_with_any", () => {
    const pattern = parsePattern(`tagged(300, [{*: *}, (*)*])`);
  ...
  it("test_extreme_nesting_depth", () => {
    ...
    const pattern = parsePattern(`tagged(400, {"level1": {"level2": {"level3": [42]}}})`);
