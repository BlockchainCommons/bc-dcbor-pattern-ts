...
describe("array detailed tests", () => {
  it("test_array_pattern_paths_with_captures", () => {
    ...
    const innerPattern = parsePattern("[@item(42)]");
...
[42]`;
    ...
    const elementPattern = parsePattern("@item(42)");
...
[42, 100, 200]`;
  ...
  it("test_array_pattern_nested_structure", () => {
    ...
    const pattern = parsePattern("[@outer_item([@inner_item(number)])]");
...
[42, 100, 42]`;
  ...
  it("test_array_pattern_mixed_types", () => {
    ...
    const pattern = parsePattern("[@any_item(*)]");

    const [paths, captures] = getPathsWithCaptures(pattern, cborData);

    // The reference lists the element paths among the match paths
