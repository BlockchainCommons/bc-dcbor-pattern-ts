/**
 * Partial parsing: the pattern at the start of a longer text and how much of
 * it was consumed, the whitespace after the pattern included.
 */

import { describe, it, expect } from "vitest";
import { tryParsePattern, tryParsePatternPartial, display } from "../src";

describe("parse partial tests", () => {
  it("test_parse_partial_basic", () => {
    const result = tryParsePatternPartial("true rest");
  ...
  it("test_parse_partial_with_whitespace", () => {
    const result = tryParsePatternPartial("42    more stuff");
    expect(result.ok).toBe(true);
    if (result.ok) {
      const { pattern: pattern, length: consumed } = result.value;
  ...
  it("test_parse_partial_complete_input", () => {
    const result = tryParsePatternPartial("false");
    expect(result.ok).toBe(true);
    if (result.ok) {
      const { pattern: pattern, length: consumed } = result.value;
  ...
  it("test_parse_partial_complex_pattern", () => {
    const result = tryParsePatternPartial("number | text additional");
    expect(result.ok).toBe(true);
    if (result.ok) {
  ...
  it("test_parse_partial_with_valid_following_token", () => {
    const result1 = tryParsePatternPartial("true false");
    ...
    if (result1.ok) {
      ...
      const result2 = tryParsePatternPartial(remaining);
  ...
  it("test_parse_partial_error_cases", () => {
    ...
    const invalidResult = tryParsePatternPartial("invalid_pattern");
    expect(invalidResult.ok).toBe(false);

    // Empty input should error
    const emptyResult = tryParsePatternPartial("");
