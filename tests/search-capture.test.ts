/**
 * Search capture tests.
 */

import { describe, it, expect } from "vitest";
import { cbor, parsePattern, getPathsWithCaptures, getPaths } from "./common";
...
describe("search capture tests", () => {
  ...
  it("test_search_capture_multiple_matches", () => {
    const pattern = parsePattern("search(@target(42))");
  ...
  it("test_search_capture_nested_structure", () => {
    const pattern = parsePattern(`search(@deep("target"))`);
  ...
  it("test_search_capture_with_array_elements", () => {
    const pattern = parsePattern("search(@item(array))");
  ...
  it("test_search_capture_no_match", () => {
    const pattern = parsePattern("search(@notfound(999))");
    const cborData = cbor([1, [2, 42], 3]);
  ...
  it("test_search_capture_complex_pattern", () => {
    const pattern = parsePattern(`search(@found({"id": @id_value(number)}))`);
    const cborData = cbor({
      users: [
        { id: 1, name: "Alice" },
        { id: 2, name: "Bob" },
  ...
  it("test_search_capture_api_consistency", () => {
    const pattern = parsePattern("search(@item(42))");
