 ...
 * Performance tests.
 *
 * Performance bounds in this file are deliberately about five times the reference's ceilings to
...
describe("performance tests", () => {
  it("test_deeply_nested_performance", () => {
    ...
    const pattern = parsePattern(`tagged(100, {"a": {"b": {"c": {"d": [42]}}}})`);
    ...
    // Performance should be reasonable (five times the reference's 10ms bound)
  ...
  it("test_complex_repeat_pattern_performance", () => {
    ...
    const pattern = parsePattern(`[({"id": number})*, (*)*, ({"name": text})*]`);
    const patternCreationTime = performance.now() - patternStart;
    ...
    // Performance should be reasonable even with backtracking (five times the reference's 10ms bound)
    expect(patternCreationTime).toBeLessThan(50);
    expect(matchTime).toBeLessThan(50);
    expect(pathsTime).toBeLessThan(50);
  });
  ...
  it("test_large_array_with_search_performance", () => {
    ...
    const pattern = parsePattern(`search("needle")`);
    const patternCreationTime = performance.now() - patternStart;
    ...
    // Search performance should scale reasonably (five times the reference's 10ms / 20ms bound)
    expect(patternCreationTime).toBeLessThan(50);
  ...
  it("test_complex_or_pattern_performance", () => {
    ...
    // OR pattern performance should be reasonable (five times the reference's 10ms bound)
    expect(patternCreationTime).toBeLessThan(50);
    expect(matchTime).toBeLessThan(50);
    expect(pathsTime).toBeLessThan(50);
  });
  ...
  it("test_vm_instruction_optimization", () => {
    ...
    // Multiple complex matches should complete quickly (five times the reference's 20ms bound)
  ...
  it("test_edge_case_performance", () => {
    ...
    // Should not exhibit exponential behavior (five times the reference's 50ms bound)
