 ...
 *
 * Usage:
 *   bun scripts/api-report.ts --local   # (re)generate api/dcbor-pattern.api.md
 ...
 * (api/dcbor-pattern.api.md) is the reviewable record of the public surface: every
...
try {
  ...
  if (result.apiReportChanged && !local) {
    console.error(
      "Public API surface changed but api/dcbor-pattern.api.md was not updated.\n" +
  ...
  console.log(
    local
      ? "API report (api/dcbor-pattern.api.md) is up to date."
